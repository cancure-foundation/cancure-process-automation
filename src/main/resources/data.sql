...
insert into settings values (32, 'CampTestReport_email', '<div style="border : 2px solid #f4961c;"><div style="background-color: #f4961c;color: #fff;padding:8px 15px;font-weight:600;">Cancure Foundation</div><div style="padding:15px;color: #222d32;font-weight:500;">Greetings from Cancure, <br><br>We are sending you the Lab test results of "${patientName}" who attended our Camp at ${campVenue} on ${campDate}.<br> <br><table border=1 style="border-collapse: collapse;"><tr><th style="padding:4px 8px;"> PRN</th><th style="padding:4px 8px;"> Patient Name</th><th>Age</th><th>Gender</th><th>Phone</th></tr><tr><td style="padding:4px 8px;">${patientUID}</td><td style="padding:4px 8px;">${patientName}</td><td>${patientAge}</td><td>${patientGender}</td><td>${patientPhone}</td></tr></table><br> <br> <b>Tests Conducted -</b><ul><#list patientTestNames as item><li>item</li></#list></ul>Test results are attached with this email.<br><br><b>Thanks,</b> <br>Cancure</div></div>');
