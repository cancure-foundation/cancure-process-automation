		...
		.controller(
				...
				[
						...
						function($rootScope, $scope, $state, $http, $cookies,
								Flash, appSettings, apiService, $mdDialog,
								$mdMedia) {
							...
							var init = function() {
								if ($cookies.get('userName')) {
									appSettings.loginUserName = $cookies
											.get('userName'); // sets the
									// userName to
									// app settings
									appSettings.roles = JSON.parse($cookies
											.get('roles')); // sets the roles to
									// app settings
									appSettings.access_token = $cookies
											.get('access_token'); // sets the
									// access
									// token to
									// app
									// settings
									$http.defaults.headers.common.Authorization = 'Bearer '
											+ $cookies.get('access_token'); // sets
									// the
									// access
									// token
									// for
									...
									Flash.create('warning', 'Please log in !',
											'large-text');
									$state.go('login');
									return;
								}

								// to set the height for center content so as to
								// enable the scroll
								...
								$rootScope
										.$on(
												...
												function(event, toState,
													...
													// so that previous page
													// scroll is not retained
													var centerContent = document
															.getElementById('center-content-wrapper');
													if (centerContent)
														centerContent.scrollTop -= centerContent.scrollTop; // scrolls
													// each
													// view
													// to
													// top
								...
								angular
										.forEach(
												...
												function(v, k) {
													...
													angular
															.forEach(
																	appSettings.pageAccess[v.authority],
																	function(
																			val,
																			key) {
																		if (accessList
								...
								for (var i = 0; i < accessList.length; i++) {
									vm.menuItems
											.push(appSettings.menuList[accessList[i]]);
								}
							}
							...
							// control sidebar open & close in mobile and normal
							// view
							...
							vm.menuActive = function(pageId) {
								var pageList = {
									0 : [ 'app.home' ],
									1 : [ 'app.manageUser.createUser',
											'app.manageUser.userList' ],
									2 : [ 'app.patientRegistration' ],
									3 : [ 'app.hospital.hospitalList',
											'app.hospital.hospitalCreate',
											'app.hospital.hpocMapping' ],
									4 : [ 'app.doctor' ],
									5 : [],
									6 : [ 'app.lab.labList',
											'app.lab.labCreate' ],
